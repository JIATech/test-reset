...
def has_reset_condition(level_number):
    """Check if a window title matches the reset condition."""
    # result = f"Level: {level_number} ||" in window_title  
...
def has_reset_and_master_reset_condition(level_number, reset_number):
    ...
    if (level_number == range(380, 401)) and (reset_number == range(10, 50)):
        print()
        print(f"Condiciones para reset se cumplen.\nLevel: {level_number}\nReset: {reset_number}")
        print()
        return True
...
def handle_master_reset(window_title):
    try:
        ...
        window.activate()
        if not has_reset_and_master_reset_condition(number_level, range(10, 50)):
            return handle_reset(window_title)
...
while True:
    ...
    for window_title in get_window_title_by_keywords(['Character', 'Level']):
        for number_level in range(380, 401):
            ...
            elif (has_reset_condition(number_level)):
                for number_level in range(380, 401):
